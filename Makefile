...
doc/html/yajl.apdx.html: doc/html yajl.cxref
	...
	for file in $${files}; do \
		${CXREF} -xref-all -block-comments -O${.OBJDIR}/doc/html -N${PACKAGE} -I${.CURDIR}/src -I${GENHDIR} -CPP 'cc -E -U__BLOCKS__ -CC -x c' $${file}; \
	done;\
	for file in $${files}; do \
		${CXREF} -warn-all -xref-all -block-comments -O${.OBJDIR}/doc/html -N${PACKAGE} -html -html-src -I${.CURDIR}/src -I${GENHDIR} -CPP 'cc -E -U__BLOCKS__ -CC -x c' $${file}; \
	done; \
